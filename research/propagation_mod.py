...
#===============================================================================
def MAKE_PROPAG_MTRX(input_data,target_cords,detect_cords,source_cords,cpu=0):
...
def SPHERICAL_SPHERICAL(input_data,target_cords,detect_cords,source_cords,cpu=None):
    import time
    ...
    if cpu == None:
        ...
        t1 = time.time()
        H0 = np.empty((detect_cords.shape[0],target_cords.shape[0]),dtype=complex)
...
def ADD_NOISE(g0,noise_level):
    ...
    g = g0 + np.random.randn(g0.shape[0]) * np.average(np.abs(g0)) * noise_level
    return g
